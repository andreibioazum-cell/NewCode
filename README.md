...
Для больших проектов оптимизирован и редактор: строки блоков реально
переиспользуются при прокрутке, поиск строит индекс небольшими порциями без
заморозки UI, а позиции блоков находятся за O(1). Числовые переменные
скомпилированного проекта специализируются в обычные C `double`, поэтому
горячие `Forever`/`x += 1` не копируют tagged-значения. Объекты добавляются
на сцену до дорогой компиляции графических шейдеров; шейдеры догружаются по
одному после первого кадра. Добавлена нативная C-поддержка блоков
«Двигаться по дуге» (`ArcBrick`) и «Пройти через» (`GoThroughBrick`), а
русская локализация теперь содержит все переводимые ключи основного набора строк.
