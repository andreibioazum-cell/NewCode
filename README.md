...
Добавлена **3D-категория** блоков (палитра «3D»): «Создать куб/сферу/
пирамиду/цилиндр» (с именем фигуры, например `cube1`, размером и цветом),
«Поместить 3D-объект в x, y», «Изменить цвет 3D-объекта», «Удалить
3D-объект». Фигуры рисуются процедурно (изометрическая проекция) и
становятся обычными образами спрайта. ИИ-помощник умеет создавать фигуры
инструментом `create_3d_object` (с именем) и ставить все 3D-блоки через
`add_any_brick`.
